...
package com.rust.sip.GB28181.sip.header;
...
public abstract class AuthenticationHeader extends Header {
	...
	/**
	 * Creates a new AuthenticationHeader. specifing the <i>auth_scheme</i> and
	 * the vector of authentication parameters.
	 * <p>
	 * <i>auth_params</i> is a vector of String of the form <i>parm_name</i>
	 * "=" <i>parm_value</i>
	 */
	...
	/**
	 * Adds a parameter. If <i>param_name</i> belongs to <i>QUOTED_PARAMETERS</i>,
	 * <i>param_value</i> is quoted (if already not).
	 */
	 ...
	 * qop_options=new String[aux.size()]; for (int i=0; i<qop_options.length;
	 * i++) qop_options[i]=(String)aux.elementAt(i); return qop_options; }
	 */
	/** Gets the qop-options */
	...
	/*
	 * public void addQopOptionsParam(String[] qop_options) { StringBuffer
	 * sb=new StringBuffer(); for (int i=0; i<qop_options.length; i++) { if
	 * (i>0) sb.append(","); sb.append(qop_options[i]); }
