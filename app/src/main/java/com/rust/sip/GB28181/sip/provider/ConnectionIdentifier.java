...
public class ConnectionIdentifier extends Identifier {
	...
	public ConnectionIdentifier(String protocol, IpAddress remote_ipaddr,
			int remote_port) {
	...
	public ConnectionIdentifier(ConnectedTransport conn) {
		super(getId(conn.getProtocol(), conn.getRemoteAddress(), conn
				.getRemotePort()));
	}

	/** Gets the id. */
	private static String getId(String protocol, IpAddress remote_ipaddr,
			int remote_port) {
