 ...
 * associated to the corresponding method type, and finally for a SipInterface
 * associated to ANY messages. If the present SipInterface id matches, the
 * SipInterfaceListener method <i>onReceivedMessage()</i> is fired.
 */
public class SipInterface implements SipProviderListener {
	...
	/** Creates a new SipInterface. */
	public SipInterface(SipProvider sip_provider, Identifier id,
			SipInterfaceListener listener) {
		this.sip_provider = sip_provider;
		this.listener = listener;
		this.id = id;
