...
public class SipParser extends Parser {
	...
	public static boolean isMark(char c) { // return (c=='-' || c=='_' ||
		// c=='.' || c=='!' || c=='~' ||
		// c=='*' || c=='\'' || c=='|');
	 ...
	 * include '\r' and '\n'
	 */
	public static boolean isSeparator(char c) { // return (isSpace(c) ||
		// isCRLF(c) || c=='(' || c==')'
		// || c=='<' || c=='>' || c==','
		// || c==';' || c=='\\' ||
		// c=='"' || c=='/' || c=='[' ||
	...
	public int indexOfEOH() {
		...
		while (true) {
			...
			if (!par.hasMore())
				return str.length(); // if no CR/LF found return the end of
			// string
			int end = par.getPos();
	...
	/**
	 * Skips the first NameAddress. If no NameAddress is found, it goes to the
	 * end of the string.
	 */
	/*
	 * public SipParser skipNameAddress() { int begin_url=indexOf("<sip:"); if
	 * (begin_url<0) begin_url=indexOf("sip:"); if (begin_url<0)
	 * index=str.length(); else { goTo(uri_separators);
	 * skipChars(uri_separators); } return this; }
	 */

	/**
	 * Returns the first quoted string in the string <i>str</i>; if no
	 * NameAddress is found, it returns <b>null</b>
	 */
	/*
	 * public String parseQuotedString() { int begin=str.indexOf('\"',index); if
	 * (begin<0) return null; begin++; int end=str.indexOf('\"',begin); String
	 * quotedtext=str.substring(begin,end); index=end; return quotedtext; }
	 */
	...
	public Date getDate() {
		...
		// 'GMT'",Locale.US);
		try { // Date d=df.parse(str,new ParsePosition(index));
	 ...
	 */
	public Message getSipMessage() { // skip any CRLF sequence
		skipCRLF();
		...
		if (!hasMore())
			return null;
		int body_pos = skipChar().getPos(); // skip the LF of the empty line and
		// go the the body
