 ...
 * PRG (Pseudo-Random-Generator) function in OFB (Output Feadback) to genarate a
...
public class MD5OTP {
	...
	/*
	 * public MD5OTP(int bsize, byte[] key, byte[] iv) { init(bsize,key,iv); }
	 */
	...
	public byte[] update(byte[] m) {
		...
		for (int i = 0; i < m.length; i++) {
			index++;
			if (index == size) { // calculate new h_block
				h = hash(cat(skey, h));
				index = 0;
			}
	...
	public static byte[] encrypt(byte[] m, byte[] key) { // choose a random
		// IV
	...
	public static byte[] decrypt(byte[] c, byte[] key) { // read the IV
		byte[] iv = sub(c, 0, 16);
