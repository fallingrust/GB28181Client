...
public class Mangle {
	...
	/**
	 * Returns a <i>len</i>-byte array from array <i>b</i> with offset
	 * <i>offset</i>
	 */
	...
	/**
	 * Copies a the first 4 bytes of array <i>src</i> into array <i>dst</i>
	 * with offset <i>index</i>
	 */
	...
	/**
	 * Transforms the first <i>len</i> bytes of an array into a string of hex
	 * values
	 */
	 ...
	 * <i>len</i>. The string may include ':' chars. If <i>len</i> is set to
	 * -1, all string is converted.
	 */
	public static byte[] hexStringToBytes(String str, int len) { // if the
		// string is
		// of the
		// form
		// xx:yy:zz:ww..,
		...
		for (int i = 0; i < len; i++) {
			if (len < str.length() / 2)
				b[i] = (byte) Integer.parseInt(str.substring(i * 2, i * 2 + 2),
						16);
			else
				b[i] = 0;
		}
	...
	public static String bytesToAddress(byte[] b) {
		return Integer.toString(uByte(b[0])) + "."
				+ Integer.toString(uByte(b[1])) + "."
				+ Integer.toString(uByte(b[2])) + "."
	...
	public static long bytesToInt(byte[] b) {
		return ((((((long) uByte(b[0]) << 8) + uByte(b[1])) << 8) + uByte(b[2])) << 8)
				+ uByte(b[3]);
	}
	...
	public static long bytesToWord(byte[] b) {
		return ((((((long) uByte(b[3]) << 8) + uByte(b[2])) << 8) + uByte(b[1])) << 8)
				+ uByte(b[0]);
	}
