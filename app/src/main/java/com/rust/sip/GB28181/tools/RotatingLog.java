...
public class RotatingLog extends Log {
	...
	/**
	 * Creates a new RotatingLog file <i>filename</i> . RotatingLog size is
	 * limited to <i>logsize</i> [bytes]
	 */
	public RotatingLog(String filename, String logname, int loglevel,
			long logsize, int n_rotations, int t_scale, int t_value) {
	...
	public RotatingLog rotate() {
		if (num_rotations > 0) {
			for (int i = num_rotations - 2; i > 0; i--) { // rename back files
	 ...
	 * <i>verbose_level</i>
	 */
	public Log print(String message, int level) { // long
		// now=GregorianCalendar.getInstance().getTime().getTime();
	...
	private void updateNextRotationTime() { // Calendar
