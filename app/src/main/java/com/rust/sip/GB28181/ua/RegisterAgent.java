...
import com.rust.sip.GB28181.sip.dialog.SubscriberDialog;
import com.rust.sip.GB28181.sip.dialog.SubscriberDialogListener;
...
import com.rust.sip.GB28181.sip.transaction.MESSAGETransactionClient;
import com.rust.sip.GB28181.sip.transaction.TransactionClient;
import com.rust.sip.GB28181.sip.transaction.TransactionClientGB28181Listener;
import com.rust.sip.GB28181.sip.transaction.TransactionClientListener;
 ...
 */
public class RegisterAgent implements TransactionClientListener, SubscriberDialogListener, TransactionClientGB28181Listener {
	...
	public boolean register(int expire_time) {
		...
		if (expire_time > 0)
		{
			Logger.d("Registering contact " + contact + " (it expires in "
					+ expire_time + " secs)");
		...
		{
			Logger.d("Unregistering contact " + contact);
			printLog("Unregistering contact " + contact, LogLevel.HIGH);
		}
		
		t = new TransactionClient(sip_provider, req, this, 30000);
		mt=new MESSAGETransactionClient(sip_provider,this,this.user_profile);
	...
	public void onTransGB28181SuccessResponse(MESSAGETransactionClient tc, Message resp) {
		if (listener != null)
		{
			listener.onUaRegistrationGb28181Success(resp.getFromHeader().getNameAddress());
		}
	}
